...
# Set page title
st.title("Lexington and Arlington Budget Chatbot")
...
with open("./scripts/arlington_vector_metadata.json", 'r') as file:
    arlingtonMetadata = json.load(file)
...
arlingtonMetadata = dict(sorted(arlingtonMetadata.items(), key=getUpdatedTime, reverse=True))
...
print(type(brownBookCollection))
schoolCollection = Chroma(persist_directory="chromadb", embedding_function=embeddings, collection_name="lc_chroma_schoolbudget")
arlingtonBudgetCollection = Chroma(persist_directory="chromadb_arlington", embedding_function=embeddings, collection_name="lc_chroma_arlingtonbudget")
...
def getVectorText(collection, metadata, rephrasedQuery, years, docPath):
...
metadata filter: 
    ...
    for index, doc in enumerate(top_vectors):
        ...
        link = "<a href='" + docPath + source + "#page=" + page + "'>" + source + " (page(s) " + page + " to " + end_page + ")</a>"
...
class SchoolTool(BaseTool):
    ...
    description = "Good for answering questions about the Lexington school or education budget for " + ', '.join(getSchoolDocYears()) + ". For budget inquiries pertaining to other years, we recommend utilizing the General Budget Search tool."
    ...
    {query}\n
        ...
        return getVectorText(schoolCollection, schoolMetadata, rephrasedQuery, years, "https://brainana.github.io/LexBudgetDocs/")
...
class GeneralBudgetTool(BaseTool):
    ...
    def _helper(self, query):
        ...
        return getVectorText(self.collection, self.metadata, rephrasedQuery, years, self.docs_url)
...
brownBookTool = GeneralBudgetTool(
    name="general_budget_search",
    description="Good for answering general budget questions related to Lexington.",
    collection=brownBookCollection,
    metadata=metadata,
    docs_url="https://brainana.github.io/LexBudgetDocs/"
)
arlingtonBudgetTool = GeneralBudgetTool(
    name="arlington_budget_search",
    description="Good for answering budget questions related to Arlington.",
    collection=arlingtonBudgetCollection,
    metadata=arlingtonMetadata,
    docs_url="https://cassidyxu.github.io/ArlingtonBudgetDocs/"
)
tools = [schoolTool, brownBookTool, arlingtonBudgetTool]
...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are a financial assistant that is very knowledgable on the budget of the towns of Lexington and Arlington. If the
            query does not specify a town, default to Lexington.
