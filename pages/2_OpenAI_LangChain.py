...
st.markdown('Using LangChain framework + ChromaDB + LangChain chain')
...
if "GPTChainMessages" not in st.session_state:
    st.session_state.GPTChainMessages = []

# Display all previous messages upon page refresh
assistantAvatar = config.get('Template', 'assistantAvatar')
numMsgs = len(st.session_state.GPTChainMessages)
for index,message in enumerate(st.session_state.GPTChainMessages):
        ...
        if message["role"] == "assistant":
            with st.chat_message(message["role"], avatar=assistantAvatar):
                ...
                if index < numMsgs - 1:
                    ...
                    if st.session_state.GPTChainMessages[index+1]["type"] == "reference":
                        with st.expander("References"):
                            st.markdown(st.session_state.GPTChainMessages[index+1]["content"], unsafe_allow_html=True)
        else:
...
# see https://github.com/langchain-ai/langchain/blob/master/templates/rag-chroma/rag_chroma/chain.py
...
# set up RAG chain, which will fill in template + query model -> generate output
chain = (
...
if prompt := st.chat_input(chatInputPlaceholder):
    ...
    st.session_state.GPTChainMessages.append({"role": "user", "type": "message", "content": prompt})
    ...
    with st.chat_message("assistant", avatar=assistantAvatar):
        ...
        st.session_state.GPTChainMessages.append({"role": "assistant",  "type": "message", "content": full_response})
