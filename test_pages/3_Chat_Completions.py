...
if "GPTStreamMessages" not in st.session_state:
    st.session_state.GPTStreamMessages = []

# Display all previous messages upon page refresh
assistantAvatar = config.get('Template', 'assistantAvatar')
numMsgs = len(st.session_state.GPTStreamMessages)
for index,message in enumerate(st.session_state.GPTStreamMessages):
        ...
        if message["role"] == "assistant":
            with st.chat_message(message["role"], avatar=assistantAvatar):
                ...
                if index < numMsgs - 1:
                    ...
                    if st.session_state.GPTStreamMessages[index+1]["type"] == "reference":
                        with st.expander("References"):
                            st.markdown(st.session_state.GPTStreamMessages[index+1]["content"], unsafe_allow_html=True)
        else:
...
if prompt := st.chat_input(chatInputPlaceholder):
    ...
    st.session_state.GPTStreamMessages.append({"role": "user", "type": "message", "content": prompt})
    ...
    with st.chat_message("assistant", avatar=assistantAvatar):
        ...
        st.session_state.GPTStreamMessages.append({"role": "assistant",  "type": "message", "content": full_response})
